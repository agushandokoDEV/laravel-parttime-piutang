...
class LaporanController extends Controller
{
    ...
    public function export()
    {
        ...
        $headerData = [
            ['SKPD', 'Nama', 'Usulan Ke BPKD', '', '', 'Rincian Piutang', '', '', 'Usulan PUPN', '', 'Balasan PUPN', '', 'Pembayaran', '', 'Keputusan Gubernur', '', 'Status'],
        ...
        $data = DB::table('surat_usulans')
            ...
            ->leftJoin('berita_acaras', 'berita_acaras.id', 'surat_usulans.id')
            ->leftJoin('pembayarans', 'pembayarans.id', 'surat_usulans.id')
            ->select('surat_usulans.*', 'jenis_piutangs.jenis', 'surat_usulan_knkpls.*', 'users.*', 'surat_balasan_knkpls.*', 'keputusan_gubernurs.*', 'berita_acaras.judul', 'pembayarans.*')
            ->get();
        $row = 3;
        foreach ($data as $value) {
            if ($value->docs_balasan != null) {
                ...
                $sheet->setCellValue('B' . $row, $value->nama_peminjam);
